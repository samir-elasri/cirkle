	...
	public static function clean_filename($filename, $source_langcode = NULL)
	{
		if (is_array($filename)) {
			foreach ($filename as $key => $value) {
				$filename[$key] = self::clean_filename($value, $source_langcode);
			}
			return $filename;
		}

		$filename = self::transliteration_process($filename, '', $source_langcode);
		// Replace whitespace.
		...
		foreach ($matches[0] as $str) {
			...
			for ($i = -1; --$len;) {
				...
				if ($remaining = $tail_bytes[$c]) {
					...
					$result .= self::transliteration_replace($ord, $unknown, $source_langcode);
					$head = '';
				} elseif ($c < "\x80") {
					// ASCII byte.
					$result .= $c;
