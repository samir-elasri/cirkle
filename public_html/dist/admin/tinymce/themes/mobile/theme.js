...
-     */
	...
	define(
		...
		function(FieldPresence, FieldSchema, ValueSchema, Adt, Fun, Id, Option) {
			...
			var fFactory = FieldSchema.defaulted('factory', {
				sketch: Fun.identity
			});
	...
	define(
		...
		function(Objects, Arr, Merger, Element, Node, Html, Traverse, Array) {
			...
			var fromHtml = function(html) {
				...
				var contents = children.length === 0 ? {} : {
					innerHtml: Html.get(elem)
				};
	...
	define(
		...
		function(AlloyTriggers, SliderModel, Fun, Option, PlatformDetection, Math) {
			...
			var fireChange = function(component, value) {
				AlloyTriggers.emitWith(component, changeEvent, {
					value: value
				});
			};
			...
			var sketch = function(rawSpec) {
				...
				var navigationButton = function(direction, directionName, enabled) {
					return Button.sketch({
						...
						action: function(button) {
							AlloyTriggers.emitWith(button, navigateEvent, {
								direction: direction
							});
						},
				...
				var memForm = Memento.record(
					Form.sketch(function(parts) {
						return {
							...
							components: [
								Container.sketch({
									dom: UiDomFactory.dom('<div class="${prefix}-serialised-dialog-chain" style="left: 0; position: absolute;"></div>'),
	...
	define(
		...
		function(Behaviour, ActiveTransitioning, TransitionApis, TransitionSchema, Objects, Obj) {
			...
			var createBistate = function(first, second, transitions) {
				return Objects.wrapAll([{
						key: first,
						value: Objects.wrap(second, transitions)
					},
					{
						key: second,
						value: Objects.wrap(first, transitions)
					}
				]);
			};
	...
	define(
		...
		function(ObjIndex, DomModification, Objects, Arr, Obj, Merger, Json, Fun, Result) {
			...
			var duplicate = function(aspect, k, obj, behaviours) {
				return Result.error('Multiple behaviours have tried to change the _' + k + '_ "' + aspect + '"' +
		...
		function(
			...
			var build = function(rawUserSpec) {
				...
				return GuiTypes.getPremade(rawUserSpec).fold(function() {
					var userSpecWithUid = Merger.deepMerge({
						uid: Tagger.generate('')
					}, rawUserSpec);
		...
		function(Focusing, AlloyTriggers, Fun, Focus) {
			...
			var onHover = function(item) {
				...
				if (Focus.search(item.element()).isNone() || Focusing.isFocused(item)) {
					...
					AlloyTriggers.emitWith(item, hoverEvent, {
						item: item
					});
				}
			};
			...
			var onFocus = function(item) {
				AlloyTriggers.emitWith(item, focusEvent, {
					item: item
				});
			};
					...
					events: AlloyEvents.derive([
						...
						AlloyEvents.run(ItemEvents.focus(), function(menu, simulatedEvent) {
							...
							menu.getSystem().getByDom(event.target()).each(function(item) {
								...
								AlloyTriggers.emitWith(menu, MenuEvents.focus(), {
									menu: menu,
									item: item
								});
							});
		...
		function(DelayedFunction, NativeEvents, SystemEvents, Objects, Cell, Fun, Option, Compare, Math) {
			...
			var monitor = function(settings) {
				...
				var handlers = Objects.wrapAll([{
						key: NativeEvents.touchstart(),
						value: handleTouchstart
					},
					{
						key: NativeEvents.touchmove(),
						value: handleTouchmove
					},
					{
						key: NativeEvents.touchend(),
		...
		function(FieldSchema, ValueSchema, Fun, Element, Traverse, window) {
			return ValueSchema.objOf([
				FieldSchema.strictObjOf('editor', [
					...
					FieldSchema.defaulted('onScrollToCursor', Fun.constant({
						unbind: Fun.noop
					})),
					FieldSchema.defaulted('onScrollToElement', Fun.constant({
						unbind: Fun.noop
					})),
					FieldSchema.defaulted('onToEditing', Fun.constant({
						unbind: Fun.noop
					})),
					FieldSchema.defaulted('onToReading', Fun.constant({
						unbind: Fun.noop
					})),
