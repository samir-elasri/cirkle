<div class='field Generated-field'>
    ...
    <div class='field-body'>
		<div class='field'>
			<div class='control'>
                <input {{ !empty($errorMessage) ? "aria-describedby='error-" . $name . "'" : '' }} {{ !empty($requiredString) ? 'aria-required="true"' : '' }} id="{{ $name }}" value='{{$value}}' class='input {{$errorClass}}' type='number' placeholder='' name='{{$name}}'>
				{!!$errorMessage!!}
			</div>
		</div>
	</div>
