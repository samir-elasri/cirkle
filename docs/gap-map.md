...
- **MASTER 2350 fidelity — importer DONE (2026-06-13):** `ExcelImport` rewritten for the 010626 layout (the old one read the wrong columns). Captures per-fragment colours/bold as HTML (`formatted_title`), strips red (spec: «disparaît»), preserves literal spaces, flags col-D «X» as `has_input`, stores `source_row` (vertical-gap reconstruction), reads «Clientèle cible» → `service_categories.provider_type`, forfait prices 1/3/6/12, keywords. **Remaining for #7:** public-fiche literal rendering (pre-wrap + colors) and the supplier tick UI. **Not yet modelled (file sections deliberately skipped):** fee-gate block (feature #6), provincial subscription prices, supplier-website tiers — all present in the MASTER file rows 180-377.
